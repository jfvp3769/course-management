...
/* =========================================================
    ...
    function requestRemoveSubject(courseCode) {
      showConfirmation(
        ...
        'This will remove ' + courseCode + ', all its sections, weekly schedules, and planned lesson records.',
        () => {
    ...
    function requestRemoveSection(courseCode, section) {
      showConfirmation(
        ...
        'Remove section ' + section + ' from ' + courseCode + ' and clear its scheduled periods?',
        () => {
          const sub = courseData.subjects.find(s => s.code === courseCode);
          if (sub) {
    ...
    function openExportImportModal(defaultTab = null) {
      ...
      if (secSelect) {
        ...
        (courseData.subjects || []).forEach(sub => {
          (sub.sections || []).forEach(sec => sections.add(sec));
        });
        studentRoster.forEach(s => { if (s.section) sections.add(s.section); });

        secSelect.innerHTML = '<option value="">All Sections (' + studentRoster.length + ' Students)</option>' +
          Array.from(sections).map(sec => `<option value="${escapeHtml(sec)}">Section: ${escapeHtml(sec)}</option>`).join('');
      }
