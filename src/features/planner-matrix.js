    ...
    if (weekendEntry) {
      ...
      return `
            <td id="cell-${cellKey}" data-cell-key="${cellKey}" data-col="${colKey}" style="width: ${currentW}px; min-width: ${currentW}px; max-width: ${currentW}px;" class="p-1 border-r border-slate-200 dark:border-slate-800 bg-[#e8edf3] dark:bg-[#182234] cursor-pointer transition align-top relative group/card-cell matrix-cell-slot" data-action="openLessonModal" data-date="${d.dateKey}" data-course="${escapeHtml(sub.code)}" data-section="${escapeHtml(sec)}" data-weekend="${d.isWeekend ? 'true' : 'false'}" data-action-dragover="handleMatrixCellDragOver" data-action-dragleave="handleMatrixCellDragLeave" data-action-drop="handleMatrixCellDrop">
              <div id="card-${cellKey}" draggable="true" data-action-dragstart="handleMatrixCardDragStart" data-action-dragend="handleMatrixCardDragEnd" data-cell-key="${cellKey}" data-date="${d.dateKey}" data-course="${escapeHtml(sub.code)}" data-section="${escapeHtml(sec)}" class="p-1 rounded-lg border ${badgeColor} ${accentBarClass} shadow-xs space-y-0.5 overflow-hidden max-w-full min-w-0 transition-all duration-200 ease-out group-hover/card-cell:absolute group-hover/card-cell:left-1 group-hover/card-cell:right-1 group-hover/card-cell:top-1 group-hover/card-cell:z-22 group-hover/card-cell:shadow-xl group-hover/card-cell:scale-[1.02] ${badgeColor} cursor-grab active:cursor-grabbing">
    ...
    if (entry) {
      ...
      return `
            <td id="cell-${cellKey}" data-cell-key="${cellKey}" data-col="${colKey}" style="width: ${currentW}px; min-width: ${currentW}px; max-width: ${currentW}px;" class="p-1 border-r border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 transition align-top relative group/card-cell matrix-cell-slot" data-action="openLessonModal" data-date="${d.dateKey}" data-course="${escapeHtml(sub.code)}" data-section="${escapeHtml(sec)}" data-weekend="${d.isWeekend ? 'true' : 'false'}" data-action-dragover="handleMatrixCellDragOver" data-action-dragleave="handleMatrixCellDragLeave" data-action-drop="handleMatrixCellDrop">
              <div id="card-${cellKey}" class="p-1 rounded-lg border border-dashed border-slate-300 dark:border-slate-700 hover:border-msu-maroon text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-100 bg-slate-50/60 dark:bg-slate-900/50 hover:bg-white dark:hover:bg-slate-800 text-center ${accentBarClass} transition-all duration-200 group-hover/card-cell:absolute group-hover/card-cell:left-1 group-hover/card-cell:right-1 group-hover/card-cell:top-1 group-hover/card-cell:z-22 group-hover/card-cell:shadow-lg group-hover/card-cell:scale-[1.02] group-hover/card-cell:bg-white dark:group-hover/card-cell:bg-slate-800">
